...
stow git vim zsh claude tmux ghostty opencode
zsh -l
...
利用する Stow パッケージは `git`、`vim`、`zsh`、`claude`、`tmux`、`ghostty`、`opencode` です。

このリポジトリにはアプリケーションのような build / test / lint の共通コマンドはありません。単体テスト実行コマンドも定義されておらず、通常は対象パッケージを `stow` し直し、必要に応じてシェルや各ツールをリロードして確認します。

## 高レベルアーキテクチャ

- リポジトリ全体は Stow パッケージとして構成されており、`git/.gitconfig`、`tmux/.tmux.conf`、`vim/.vimrc`、`zsh/.zprofile`、`zsh/.zshrc`、`claude/.claude/*`、`ghostty/.config/ghostty/*`、`opencode/.config/opencode/*` のように、配置先のホームディレクトリ構造をそのままリポジトリ内で再現しています。
- Zsh 設定は役割分担されています。`zsh/.zprofile` は PATH や Homebrew / Go / Volta / OpenCode の初期化、`zsh/.zshrc` は補完、プロンプト、エイリアス、`direnv`、`gcloud` 連携など対話操作向けの設定を担当します。
- AI アシスタント関連の中心は `claude/.claude/` です。ここに Claude 向けの基本方針、再利用する context 文書、hooks、skills、settings が集約されています。さらに `opencode/.config/opencode/opencode.json` は `~/.claude/CLAUDE.md` を instructions として読み込むため、OpenCode も Claude の指示体系を共有します。
...
- `~/.gitconfig` や `~/.claude/CLAUDE.md` のようなホーム側の実体ではなく、必ず `git/.gitconfig`、`claude/.claude/CLAUDE.md`、`opencode/.config/opencode/*` などリポジトリ内のソースを編集します。

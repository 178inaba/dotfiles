...
**最初に `git branch --show-current` でベースブランチ名を取得して控えておく。**
...
1. **参照文書の読込**
   - プロジェクトルートの `AGENTS.md` または `CLAUDE.md` を読む
   - 文書からリンクされている `.md` ファイルを読む（標準Markdownリンク形式）
   - リンク先から更にリンクされている `.md` があれば1段だけ追跡して読む
...
6. **実装完了処理**
    ...
    - /git-pr コマンドに従ってプッシュ・PR作成
      - `gh pr create` 時は `--base <base-branch>` を付加する（計画ファイルに記録したベースブランチ）

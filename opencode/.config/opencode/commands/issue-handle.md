...
## Issue情報（自動取得）
!`gh issue view $ARGUMENTS --json title,body,labels,assignees,comments 2>/dev/null || echo "Issue情報の取得をスキップ（--file指定時）"`
...
**ファイル編集・ブランチ作成は行わず、計画の策定とユーザー承認のみを行う。**

**計画ファイル**（`~/.config/opencode/plans/` 配下、ファイル名は任意）に実装方針を記述する。
...
2. **タスク作成**
   - 方針確定後、TodoWriteツールで実装フェーズの各タスクを作成する

3. **計画完了**
   - **計画ファイルに以下を必ず含める**（コンテキストクリア後はこの計画ファイルとAGENTS.mdのみが残る）:
     - ブランチ名（typeを含む完全な形式）
...
6. **実装完了処理**
    - 未コミットの変更があれば /git-commit コマンドの方針に従ってコミット
    - /git-pr コマンドに従ってプッシュ・PR作成
...
- **コンテキストクリア後**: 計画ファイル（`~/.config/opencode/plans/`）を読み直し、ブランチ名・チェックリストを確認して作業を再開

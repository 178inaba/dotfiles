...
func newStatuslineCmd(deps Deps) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(c *cobra.Command, _ []string) error {
			return silent(statusline.Run(c.Context(), statusline.Default(),
				c.InOrStdin(), c.OutOrStdout(), deps.Build.FirstError))
		},
	}
}
...
func newRefreshCmds(deps Deps) []*cobra.Command {
	var (
		now                           int64
	...
	pr := &cobra.Command{
		...
		Args:   cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			return silent(statusline.RefreshPR(c.Context(), runner.Exec{}, deps.NewClient,
				cacheDir, cacheKey, head, dir, time.Unix(now, 0)))
		},
	}

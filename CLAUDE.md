...
- `worktree-resolution` / `finding-triage` / `finding-severity` / `fresh-reader-verification` / `pr-reading` は他スキルから起動される共有知識スキルで、ユーザー直接起動は不可
...
- 実装は `go/internal/<domain>/`（`issue` / `pullrequest` / `worktree` / `reviewprs` / `skill` / `plandocs`）、cobra 定義は `go/internal/cmd/<group>.go`

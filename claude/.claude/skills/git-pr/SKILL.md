...
8. 承認されれば、`gh pr edit` の直前に `gh pr view --json title,body` でタイトル・説明を取り直してから更新する。`gh pr edit` はどちらも全体を置き換えるので全体を渡すことになり、手順 5 の取得からユーザー確認を挟む間に入った編集は、取り直さないと巻き戻る
   - 手順 5 の写しとの差が語句・誤字の修正にとどまるなら、承認済みの文面に畳み込んでから書く
   - 節の追加・削除など内容が変わる差なら、差を示して手順 7 をやり直す
   - 手順 5 の写しが手元に無ければ手順 5 からやり直す（写しが担うのは「確認中に変わったか」の判定だけで、本文の保護は書く直前の取り直しが担う）
...
4. **テスト・確認手順**（該当する場合）: レビュアーが動作確認するためのコマンド・手順・確認ポイント。差分からは読み取れない情報のため、非自明な確認方法がある場合に書く
5. **デプロイ・設定上の注意**（該当する場合）: 新しい環境変数、マイグレーション、マージ前に必要な作業。書き漏らすと誤マージ事故に直結するため必ず記載する

UI変更を含む場合は、スクリーンショット・動画の添付をユーザーに提案する（CLIからは画像を添付できないため）。

...
8. 承認されれば、`gh pr edit` の直前にタイトル・説明を取り直し、説明は全体を渡して更新する（手順 5 の取得からユーザー確認を挟むため、その間に入った編集が全体の置き換えで巻き戻る）。手順 5 の写しと差があれば承認済みの文面に畳み込んでから書き、差が編集上のもの（承認済みの提案が残す文への語句・誤字の修正）を超えるとき — 節の追加・削除等 — は差を示して手順 7 をやり直す

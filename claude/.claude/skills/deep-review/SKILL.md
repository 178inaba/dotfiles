...
「重要なことを言う、雑音は出さない」を原則とする。誤検出・nitpick はレビュアーの信頼を損ね、本質的な指摘が埋もれる原因になる。雑音の抑制は指摘を黙って捨てる操作ではなく分離で行う — 機械的に検出できるものは下記でツールに任せ、選好レベルの指摘は nit（分類基準はセクション6）として拘束力を外して出し、指摘に昇格しない検証は検証証跡（セクション7の「確認して問題なしと判断した点」「確認した内容」）として記録する。
...
「確認して問題なしと判断した点」「確認した内容」は指摘ではなく検証証跡 — 再レビュー時に同じ懸念を蒸し返さないための記録であり、前提誤り（「backfill 不要の理解で見ています」等）を作者が訂正できる窓口でもある。指摘に昇格させない検証はここに置く。

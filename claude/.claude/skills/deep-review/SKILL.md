...
- **nit は `comments[]` に入れず、`body` 末尾に `<details>` で折りたたんで集約する**（行コメントはスレッド化してセクション9の確認・resolve 管理の対象になり、non-blocking（対応は任意）の意味論と合わないため。折りたたみは nit のみ — 必須修正・推奨修正・質問は読み飛ばされては困るので折りたたまない）:
  ...
  - `<details>` を使うのはこの投稿 body のみ。セクション7のローカル出力には入れない（HTML タグは端末表示では単なるノイズ）

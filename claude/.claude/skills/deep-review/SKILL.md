...
- **折りたたみの原則**: 作者のアクションが不要な内容（コメント内の長い根拠・検証記録・nit）は `<details>` で折りたたみ、アクションを要求する内容は折りたたまない。`<details>` は PR への投稿（body・行コメント）専用 — セクション7のローカル出力には入れない（HTML タグは端末表示では単なるノイズ）。`<summary>` 行の直後の空行は必須（無いと中身の Markdown がレンダリングされない）
...
- **`body` の「確認して問題なしと判断した点」「確認した内容」は `<details>` で折りたたむ**（summary に件数）

...
- [ ] コードの品質（可読性、保守性）
- [ ] バグや潜在的な問題の有無
- [ ] セキュリティリスク（範囲の正は「レビュー scope」: SAST が拾わない設計レイヤーのみ対象）
- [ ] パフォーマンスへの影響
- [ ] テストコードの妥当性（@~/.claude/skills/test-implementation/SKILL.md の3原則: 無駄なテスト・抜け漏れ・可読性）
- [ ] エラーハンドリングの適切性
- [ ] 命名規則やコーディングスタイルの一貫性（「レビュー scope」が除外する軽微な揺れは対象外。新規追加要素はセクション4の慣習突き合わせで確認）
...
- [ ] Issue の要件・受け入れ条件を箇条書きに展開し、各項目を「充足（対応する差分箇所を特定）／未実装／逸脱」のいずれかに分類したか（結果はセクション8「Issue要件の充足状況」に反映する）
...
- 投稿フォーマット（セクション9の body 例の見出し・定型文）は構造の規定であり、見出しも含め対象 PR の言語で書く

...
- 評価は差分全体（新規追加ファイル・定数・インターフェース変更を含む）を読んだ上で行う。観点ごとの検証手続きはセクション5のトリガー式チェックリストが担う
...
- **ボールの所在（`ball`）**: 次に動くのが誰か。`"mine"` は自分が出した指摘に作者が答えたスレッドで、セクション9の返信・解決の対象になる。「返信せずコミットだけ push した」ケース（GitHub では一般的）もここに入る。自分の PR をレビューしているときは、レビュアーの指摘で自分が答える番のスレッドも `"mine"` になる（そちらはセクション9ではなくセクション10で扱う）。`"theirs"` は相手の応答待ち、`"none"` は解決済みか、他人の PR 上の他人のスレッド
...
- 返信は通常短くインラインで足りるが、コードスパン・`"` を多く含む長い返信は素の Markdown を
  `work_dir` 直下に Write して `body_file` で指す（JSON 文字列への手書きエスケープはセクション8と
  同じ理由で避ける）:

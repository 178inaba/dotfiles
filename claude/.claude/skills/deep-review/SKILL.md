...
- [ ] **「推奨」等の非必須と記載されたルールからの逸脱がある** → @~/.claude/skills/finding-triage/SKILL.md の「非必須ルールの一様準拠判定」を読み、その基準で拘束力を判定する（一様に従われていれば事実上の必須慣習として指摘対象にする — 「非必須だから違反しても Approve 可」と解釈しない）
...
**見送り判断の検証**（必須修正・推奨修正を見送る場合に必須）: @~/.claude/skills/finding-triage/SKILL.md を読み、その規律（実地検証・コスト分類・一様準拠判定・根拠記録）で検証してから見送りを確定する。写像: 必須修正・推奨修正 = 「対応が期待される指摘」、質問・確認事項 = 対象外（ユーザー確認に残すかは上記の判断基準に従う）
...
検証根拠は必須修正・推奨修正の見送りで必須（finding-triage の規律で実施した検証内容を書く。質問・確認事項を「ユーザー確認が必要」として残す場合は不要）。書けない場合は検証未実施のサインなので、判断に戻って検証してから確定する。

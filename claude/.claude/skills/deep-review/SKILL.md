...
- コメントモード → セクション6「コメントモードON時の追加要件」、セクション7「言語」、セクション9
- 個人ルールモード → セクション6「個人ルール観点」
- 自動対応モード → セクション6「自動対応モードON時の追加要件」、セクション10
...
- **新規追加されたコード要素（関数・メソッド・定数・型・フィールド等）**: 同ファイル・同クラス/struct 内の同種要素のシグネチャ慣習（引数の型と順序（特に第一引数）、エラー返却スタイル、命名パターン等）と突き合わせ、新規追加分だけが慣習から外れていないか確認。レイヤーごとに慣習が異なりうるため、新規追加を含むファイルごとに個別に実施し、同ファイル内に同種要素が無い場合（新規ファイル等）は同ディレクトリ・同レイヤーの兄弟ファイルを基準にする
...
- 新規追加要素の慣習突き合わせは、近傍の同種要素数件との比較で足りる（ファイル全体の通読・リポジトリ横断 grep は不要）。既存コードの移動・抽出でシグネチャ・実装とも既存のままの場合（純粋なリファクタリング）は対象外
...
見落としによる誤指摘（特に対応済み内容の再指摘）は、投稿すれば作者に不要な再作業を強い、`--local-only` でもユーザーの判断を誤らせる。逆に、過去レビューの存在は意識を対応確認に狭め、新規論点の見落としも生む。指摘を書く前に以下を確認する:
...
- [ ] 自分の過去レビューがある場合、前回指摘への対応確認とは独立に、前回レビュー以降の追加コミットを含む全差分へ全観点を適用したか（セクション4・6のレビューを対応箇所に限定せず実施していればこの要件は満たされる — 別途の再パスは不要）
...
- [ ] 「推奨」等の非必須と記載されたルールでも、変更対象ファイル（同種の適用箇所が無い場合は同レイヤーの周辺コード）の既存コードが一様に従っている場合は、事実上の必須慣習として逸脱を指摘対象にしたか（「非必須だから違反しても Approve 可」と解釈しない。一様性の確認は近傍の同種適用箇所数件で足り、そのすべてが従っていれば一様とみなす）
...
- [ ] `~/.claude/CLAUDE.md` からリンクされているドキュメント（`~/.claude/context/` 配下）のルールに準拠しているか
  - リンク先ドキュメントのうち、変更内容に関連するものを確認する
- [ ] 非必須と記載されたルールの逸脱を、プロジェクトルール観点の同項目と同じ基準で指摘対象にしたか

#### 基本観点
- [ ] コードの品質（可読性、保守性）
- [ ] バグや潜在的な問題の有無
- [ ] セキュリティリスク（権限管理・入力検証・認証フロー等、SAST が拾わない設計レイヤー。レビューscope の除外に従う）
- [ ] パフォーマンスへの影響
- [ ] テストコードの妥当性（@~/.claude/skills/test-implementation/SKILL.md の3原則: 無駄なテスト・抜け漏れ・可読性）
- [ ] エラーハンドリングの適切性
- [ ] 命名規則やコーディングスタイルの一貫性（レビューscope が除外する軽微な揺れは対象外。新規追加要素はセクション4の慣習突き合わせで確認）
- [ ] PR説明と実際の変更内容が整合しているか（PRが存在する場合のみ）
...
- [ ] Issue の要件・受け入れ条件を箇条書きに展開し、各項目へ対応する差分箇所・未実装・逸脱を対応付けたか（結果はセクション8「Issue要件の充足状況」に反映する）
...
### 9. コメントモードON時: PRにレビュー投稿

（セクション3の判定でコメントモードOFFの場合はスキップ。コメントモードと自動対応モードは排他）
...
（セクション3の判定で自動対応モードOFFの場合はスキップ。コメントモードと自動対応モードは排他）
...
- 非必須（「推奨」等）と記載されたルールを根拠にする見送りは、「プロジェクトルール観点」の非必須ルール項目の基準で変更対象ファイル等の一様準拠を確認してから確定する（一様に従われている場合は事実上の必須慣習であり、見送り根拠にならない）

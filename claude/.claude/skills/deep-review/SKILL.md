...
- [ ] 自分の過去レビューの各指摘について、対応状況の根拠（対応報告コメント・該当コミットの差分）を確認したか。根拠未確認のまま同じ指摘を再提起しない。**根拠の種別は区別して記録する**（差分で解消を確認できたのか、作者の報告コメントだけなのか） — セクション9の resolve 可否がこの区別を消費する
...
| 反論・議論 | 作者が意図的な設計判断・スコープ外と説明した | 根拠を添えて議論を続ける | しない |

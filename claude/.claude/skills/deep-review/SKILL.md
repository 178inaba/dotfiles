...
「重要なことを言う、雑音は出さない」を原則とする。誤検出・nitpick はレビュアーの信頼を損ね、本質的な指摘が埋もれる原因になる。雑音の抑制は指摘を黙って捨てる操作ではなく分離で行う — 機械的に検出できるものは下記でツールに任せ、選好レベルの指摘は nit（分類基準はセクション6）として拘束力を外して出す。
...
- [ ] 自分の過去レビューの各指摘について、対応状況の根拠を確認したか（根拠未確認のまま同じ指摘を再提起しない）。**コード修正で対応したと報告されたものは差分が唯一の根拠**で、報告コメントは根拠にならない（作者の自己申告のため。差分を確認するまで「対応済み」と認定せず再提起の有無も確定させない — 前回レビュー以降の差分は次項で必ず読むので追加の読解コストは発生しない）。コードに現れない対応（説明での決着・スコープ外・別Issue 化）は説明そのものが根拠になる。コメントモードON時はこの区別がセクション9の resolve 可否に写像される
...
- **重要度を明示する**: 各指摘を出力フォーマットの「必須修正／推奨修正／質問・確認事項／nit」のいずれかに分類し、どの強さの指摘かを曖昧にしない
  ...
  - **nit に落とさないもの**: 一様準拠の慣習違反（セクション5の非必須ルール項目で事実上の必須慣習と判定したもの）と、便益を根拠付きで述べられる指摘。nit への分類は指摘を消す操作ではなく拘束力を外す操作であり、「指摘の出し切り」は nit にも適用される（本物の指摘を nit に格下げすると実質的な抑制になるため）
...
「確認して問題なしと判断した点」「確認した内容」は指摘ではなく検証証跡 — 再レビュー時に同じ懸念を蒸し返さないための記録であり、前提誤り（「backfill 不要の理解で見ています」等）を作者が訂正できる窓口でもある。指摘に昇格させない検証はここに置き、黙って捨てない。
...
- **nit は総合評価に影響させない**: 判定は必須修正・推奨修正・質問・確認事項のみから導き、nit しか残っていなければ「Approve可能」とする（non-blocking の定義上、対応を求めない指摘のため）
...
- **`comments[].body` は冒頭に重要度ラベルを付け、要求を最初の1〜2文で完結させる**。ラベルは `**必須修正**:` / `**推奨修正**:` / `**質問**:`（対象 PR の言語の相当表現）— 無いと作者は行コメント単体でブロッキング性を判別できない。長い根拠（動作解説・実測値・コードスニペット等）は要求の後ろに `<details><summary>検証詳細</summary>` で折りたたむ（根拠が長いと要求自体が埋もれる）
- **行コメントにした指摘も `body` の指摘事項リストに1行で索引する**（「<指摘概要>（`path:line` に行コメント）」の形）。行コメント単独の指摘は body の件数・総合評価と食い違いを生み、作者に総合評価の枠外として軽く扱われるため
- **`body` の「確認して問題なしと判断した点」「確認した内容」は `<details>` で折りたたむ**（summary に件数。nit と同様、作者のアクションが不要な内容のため一等地に置かない）
- **nit は `comments[]` に入れず、`body` 末尾に `<details>` で折りたたんで集約する**（行コメントはスレッド化してセクション9の確認・resolve 管理の対象になり、non-blocking（対応は任意）の意味論と合わないため。指摘そのものを折りたたむのは nit だけ — 必須修正・推奨修正・質問は読み飛ばされては困るので、折りたたんでよいのは各コメント内の検証詳細に限る）:
  ...
  - `<summary>` 行の直後の空行は必須（無いと中身の Markdown がレンダリングされない）
  - 折りたたみ内は行コメントと違いコード位置にリンクされないため、作者が該当箇所へ飛べるよう `path:line` を明記する
  - `<details>` を使うのは PR への投稿（body・行コメント）のみ。セクション7のローカル出力には入れない（HTML タグは端末表示では単なるノイズ）
...
- **nit**: 対応は任意。低リスクで機械的に適用できるものだけ対応してよい。見送りに検証・理由の記録は不要で、「対応しない指摘」への列挙も不要（non-blocking の定義上、無視してよい指摘のため。レビュー出力には残っている）

...
- **確認待ちスレッド（`awaiting_my_confirmation: true`）**: 未解決 かつ 起点が自分 かつ 末尾が自分でない — 自分が出した指摘に作者が応答し、解消判定のボールが自分に戻っているスレッド。セクション9の返信・解決の対象になる
...
- [ ] 自分の過去レビューの各指摘について、対応状況の根拠を確認したか（根拠未確認のまま同じ指摘を再提起しない）。根拠は対応の種別で決まり、セクション9の返信・resolve 可否もこの区別を消費する:
  - **コード修正で対応したもの** → 該当コミットの差分が唯一の根拠。「対応しました」という報告コメントは根拠にならない（作者の自己申告のため）。差分を確認するまで「対応済み」と認定せず、再提起の有無も確定させない。前回レビュー以降の差分は次項で必ず読むため、この要求で追加の読解コストは発生しない
  - **説明・スコープ外・別Issue で対応したもの、または対応が本 PR の差分外（別 PR・依存ライブラリ側等）で確認できないもの** → 報告コメントが根拠（コードに現れないため）。確認できない旨を明示して未確定扱いにする
...
対象は `awaiting_my_confirmation: true` のスレッドのみ（判定条件はセクション5「既存レビュー考慮」の確認待ちスレッド）。0件ならスキップ:
...
| 判定 | 根拠 | 返信 | resolve |
|---|---|---|---|
| 解消 | 該当コミットの差分で解消を確認できた | 確認した旨（参照した差分箇所を添える） | する |
| 未解消・部分対応 | 差分を読んだが指摘が残っている | 残っている点を具体的に示す | しない |
| 反論・議論 | 作者が意図的な設計判断・スコープ外と説明した、または対応が本 PR の差分外で確認できない | 根拠を添えて議論を続ける | しない |

- **resolve は差分で解消を確認できた場合に限る**。「対応しました」という返信の存在は根拠にならない（作者の自己申告であり、resolve は作者の未対応リストからスレッドを消す片道の操作なので、誤 resolve は指摘の silent な喪失になる）
- **「修正報告あり・差分未確認」は終端の判定ではない**。3分岐のどれにも当てはまらないため、差分を読んで解消／未解消のいずれかに落とすまで判定を確定させない（この状態を「対応済み」と扱うと、スレッドは open のまま返信もされず、指摘がレビュー出力からも消える）
...
- `id`: `review_threads[].id`。`awaiting_my_confirmation: true` 以外を含めるとスクリプトが1件も投稿せず非ゼロ exit する（他レビュアーのスレッドの解消判定の代行・解決済みへの再投稿・二重返信を構造的に防ぐため）
- `body`: セクション6の原則・口調・言語（対象 PR の言語）で書く
- `resolve`: 上記の決定表に従う

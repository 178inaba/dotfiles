...
- **質問の前に自力検証を試みる**: 質問・確認事項は検証コストを作者へ転嫁する操作。自力で検証可能な事実（`gh` で参照できる別リポジトリの定義、コードから追える挙動・設定値）は質問にする前に検証を試み、解消したら質問ではなく「確認して問題なしと判断した点」に置く。「こちらからは確認できない」と書くのは検証を試みた後に限る。質問として残すのは、作者の意図・運用判断・レビュアーから観測できない情報（本番の負荷実績・運用計画等）に依存する部分だけ
...
- **行コメント本文（`comments[].body_file` の参照先・インラインとも）は冒頭に重要度ラベルを付ける**。ラベルは `**必須修正**:` / `**推奨修正**:` / `**質問**:`（対象 PR の言語の相当表現）— 無いと作者は行コメント単体でブロッキング性を判別できない。長い根拠（動作解説・実測値・コードスニペット等）は `<details><summary>検証詳細</summary>` に折りたたむ

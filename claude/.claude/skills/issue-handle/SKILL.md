...
6. **独立セッションでのレビュー → 親での自動修正**
   ...
   - 対応リストの確定後:
     - **「Issue 側の修正が要る指摘」が空でなければ**、以下の分岐より前に、1 指摘 1 問の `AskUserQuestion` を 1 ラウンド（5 問以上なら複数回の呼び出しで 1 ラウンド — ラウンドの定義は `grilling` が持つ）で聞き、回答で各指摘を移す。`--file` 指定時は、この区分と選択肢の「Issue」を仕様ファイルと読み替え、1 つ目では Issue 本文の代わりに仕様ファイルを書き換える

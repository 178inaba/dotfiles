...
4. **品質チェック**
   - `/simplify` を実行し、変更コードの再利用性・品質・効率性を確認・修正
     ...
     - `/simplify` の要約には、各角度が返したすべての finding を 1 件 1 行で、次の扱いのいずれか 1 つとともに列挙する（列: finding / 角度 / 扱い / 根拠）。一覧はステップ5の開始前に完成させる
       - 適用・見送り: `finding-triage` の「検証根拠の記録」が根拠を求める finding にはその根拠を添える
       - 重複: 重複先の finding をその本文で名指す
       - スコープ外: 変更コード上に無い finding
     - `/simplify` の要約はターンの終わりではない。finding の一覧の完成と修正のコミットまで済ませたら、ユーザー確認を待たず同一ターンでステップ5へ進む（`ccx hook issue-handle-guard` が強制する。バックグラウンド待ちでターンを終える規定はそのまま適用される）
   - 修正があればコミット
...
6. **独立セッションでのレビュー → 親での自動修正**
   ...
   - 起動結果の agentId を控える（6-2 の再レビューの宛先）
   ...
   - 1 周目は、レビュー結果を受け取った時刻と、6-2 の最初のコミットより前の `git rev-parse HEAD` を控える
   ...
   - 対応リストの確定後:
     ...
     1. **対応すべきものがあれば**: working tree に修正適用 → コミット → テスト・Lint → Skill ツールで `git-pr` を引数 `--base <base-branch>` で起動してプッシュ・PR更新 → 再レビューの判定へ進む
     2. **対応すべきものがゼロなら**: 修正・コミット・PR 更新は行わず再レビューの判定へ進む
   - 6-2 の中でユーザーが求めた変更は、再レビューの判定より前に分岐 1 と同じ手順（コミット → テスト・Lint → `git-pr`）で反映する。分岐 2 の周でも同じ
   - **再レビューの判定**（6-3 へ進むかはここでだけ決める）:
     - この周でプッシュしていない（ユーザーが求めた変更のプッシュも無い）、またはこの周が再レビューの結果を処理した周なら、6-3 へ進む
     - それ以外は、6-1 のレビュアーへ `SendMessage` で次を送る:
       1. 前回以降の変化: コミット範囲 `<控えた HEAD>..<現在の HEAD>` と、この周の対応リスト（対応する / 対応しない とその根拠。質問・確認事項への回答はその行に書き、ユーザー判断待ちのものはその旨を書く）
       2. Skill ツールで `deep-review` を 6-1 と同じ引数で再度起動すること
       3. このセッションで <控えた時刻> に返した自分のレビュー結果を、自分の過去レビューとして扱うこと
       4. レビュー結果をそのまま返すこと
     - 送ったら、完了通知が届くまでターンを終える（`sleep` でポーリングしない）
     - `SendMessage` がレビュアーに届かない場合は、`independent-reviewer` を 6-1 と同じ指示で新規起動し、プロンプトに前回のレビュー結果・その時刻と上記 1〜3 を加える
     - 返った結果は、本 6-2 の先頭から同じ扱いで処理する
...
- [ ] 6-2 がプッシュした場合、同じレビュアーの再レビューを 1 回実施し、その結果を 6-2 と同じ扱いで処理済み
- [ ] 同期検証を通過して PR を Ready 化済み

## 注意事項
- **PR は draft で作られる**: レビューループ（6-1〜6-2。6-2 の再レビューを含む）が終わるまで PR は draft のままで、6-3 の同期検証を通過した時点でのみ Ready 化する。エスカレーション・レビュー失敗・ユーザー中断で途中停止した場合は draft のまま残す

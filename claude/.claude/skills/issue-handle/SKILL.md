...
**Step 0. 要件確認・調査**
- 上記「### 要件確認・調査」セクションに従って Issue 本文（`!gh issue view` で取得済み）と関連コードを Read/Grep で調査する
- 目的: 続く Step 5 の worktree 名（type + description）判断材料と、Plan モード内での計画起案の初期案を確保する（Plan モード内でも追加調査可）
...
- 起動時ブランチ: `git branch --show-current`
...
- worktree 名は branch 名から `/` を `-` に置換した sanitized 形式（規約定義: @~/.claude/skills/worktree-resolution/SKILL.md の「共通規約」）
...
- 補足: 非 `--worktree` 時は本ステップ自体不要（実装フェーズの `git switch -c new origin/<base-branch>` が `origin/<base>` を直接参照するため、Step 3 の fetch だけで十分）
...
- **失敗時のリカバリ**:
  - session はまだメインツリーの cwd。Bash で `git switch <起動時ブランチ>` を実行して Step 6 で切り替えた状態を戻す
    - detached HEAD 起動時: `git switch --detach <SHA>`
...
- 実行: `git -C <メインツリー-絶対パス> switch <起動時ブランチ>`
  - detached HEAD 起動時: `git -C <メインツリー-絶対パス> switch --detach <SHA>`
- **スキップ条件**: 起動時ブランチ == ベースブランチ の場合（既に起動時と同じ状態のため復元不要）
- 失敗時: warning（「メインツリーの起動時ブランチへの復元に失敗しました。手動で戻してください」）+ 続行（worktree は正常に作成済みのため作業は継続可能）
...
名前変更を希望されたら、シンプルに worktree 破棄 → 再作成で対応する（事前準備完了直後はコミット無しのため、変更なし → セッション終了時に自動削除される。手動即時削除は `git worktree remove <path>` + `git branch -d worktree-<worktree-name>`）。
...
4. **計画完了**
   - **計画ファイルに以下を含める**:
     ...
     - 起動時ブランチ（`--worktree` & 新規シナリオ時のみ。Step 2 で記録した値、途中中断時のロールバック手掛かり）
       - detached HEAD 起動時は SHA と `detached` フラグを併記
     - メインツリー絶対パス（`--worktree` & 新規シナリオ時のみ。Step 2 で記録した値、後続の cross-worktree 操作用）
...
2. **作業ブランチ確定**
   ...
   - **非 `--worktree` 時のみ以下を実施**:
     ...
     - description ルール（モダン standard 準拠、kebab-case・feature 系は動詞から / fix 系は名詞句・60文字以内目安）
     - 例: `feature/99-add-oauth-login`, `fix/42-null-pointer`
     - `--file` 指定時（Issue 番号なし）: `<type>/<short-description>` (例: `feature/add-login-validation`)

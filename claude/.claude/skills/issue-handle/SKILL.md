...
4. **計画完了**
   - **計画ファイルに以下を含める**:
     ...
     - 実装手順チェックリスト:
       ...
       - [ ] 作業ブランチ作成（`--worktree` 指定時は worktree 作成・session 切替で兼ねる）
...
2. **作業ブランチ確定**
   ...
   **`--worktree` 指定あり**:
   ...
   - **既存 worktree の検索**: `git worktree list --porcelain` を解析し、`branch refs/heads/worktree-<worktree-name>` が登録されている worktree を探す
     - 見つかった場合: `EnterWorktree(path: <found-path>)` で session を切替（再実行・並列 session 起動時の安全策）

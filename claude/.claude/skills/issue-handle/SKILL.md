...
6. **独立セッションでのレビュー → 親での自動修正**
   ...
   - 対応リストの確定後:
     - **「Issue 側の修正が要る指摘」が空でなければ**、以下の分岐より前に、1 指摘 1 問の `AskUserQuestion` を 1 ラウンド（ラウンドの定義は `grilling`）で聞く。選択肢は「Issue を修正して指摘を適用する」（推奨として先頭）/「別 Issue として起票し、ここでは対応しない」/「Issue を維持して対応しない」。回答で各指摘を、1 つ目なら「対応する指摘」（修正方針は指摘が提案した修正）、残り 2 つなら「対応しない指摘」へ移す
       - 「Issue を修正して指摘を適用する」: 修正の適用より前に、親セッションが Issue 本文を指摘が挙げた修正で書き換え（`gh issue edit <issue-number> -R <owner/repo> --body-file <path>`。本文の既存言語で書く）、書き換えた旨を報告する
       - 「別 Issue として起票し、ここでは対応しない」: 親セッションが `gh issue create -R <owner/repo> --assignee @me --body-file <path>` で単独の Issue（現 Issue の親の Sub にしない）を起票する。本文は指摘と Issue に要る修正で、対象リポジトリの言語で書き、本構成のスキル名を出さない
       - `--file` 指定時は、この区分と選択肢の「Issue」を仕様ファイルと読み替える。1 つ目では修正の適用より前に仕様ファイルを書き換え、2 つ目の親の Sub の指定は適用しない
   ...
   6-3. **PR を Ready 化**
   - **6-2 の完了後は常に実行する**（指摘を適用・プッシュした場合も、対応すべきものがゼロだった場合も。6-2 の「Issue 側の修正が要る指摘」のラウンドがあれば、その回答を待つ）

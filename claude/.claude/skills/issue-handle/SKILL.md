...
4. **計画完了**
   - **計画ファイルに以下を含める**:
     ...
     - 実装手順チェックリスト:
       ...
       - [ ] 独立セッションでの `/deep-review` 実行（サブエージェント経由）
...
7. **独立セッションでのレビュー**
   - **目的**: 実装コンテキストを持たない独立セッションでレビューさせることで、実装時の思い込み・見落としを発見しやすくする
   ...
   - サブエージェントへのプロンプトに以下を含める:
     ...
     - 実行コマンド: `/deep-review <base-branch> --issue <issue-number> --review-only`
       ...
       - `--review-only`: 自動修正を強制OFF（修正判断は親セッションで行うため）
     - レビュー結果をそのまま返すよう指示（追加の解釈・要約は不要）
     - 補助コンテキスト: 作業ブランチ名、PR URL（既知の場合）
   - サブエージェントから返ってきたレビュー結果を親セッションで表示
   - 修正対応の有無はユーザー判断に委ねる（ユーザーが「対応する」と指示した場合のみ、親セッションで修正・コミット・プッシュを実行）

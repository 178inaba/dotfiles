...
- **worktree 名は branch 名から `/` を `-` に置換した sanitized 形式**（例: `feature/99-add-oauth-login` → `feature-99-add-oauth-login`。規約定義: @~/.claude/skills/worktree-resolution/SKILL.md の「共通規約」）。以降 Step 7 で `EnterWorktree(name:)` に渡す `<worktree-name>` はこの sanitized 名（`/` を含まないこと。`/` を含むと Claude Code 実装により `+` に置換される、2026-07-06 実測）
...
- `EnterWorktree(name: <worktree-name>)` で新規 worktree 作成（`<worktree-name>` は Step 5 の sanitized 名 = `/` を含まない形。詳細は Step 5 参照）

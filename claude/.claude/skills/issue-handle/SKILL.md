...
- `--delegate-impl`: 実装フェーズの Step 2〜3（実装・テスト・コミットのループ）と Step 6-2 の決定済み修正の適用を `model: "sonnet"` の実装エージェントに委譲する。判断（計画・レビュー指摘の要否・修正方針）と外向き操作（PR 作成・PR 説明更新・PR の Ready 化）は親セッションに残る（レビュー修正フェーズの push のみ実装エージェントが行う）
...
3. **計画完了**
   - **計画ファイルに以下を含める**:
     ...
     - 実装委譲（`--delegate-impl` 指定時 true。実装フェーズと Step 6-2 の分岐判定に使う。再開シナリオでも今回の起動引数を正とし、前回計画の値は引き継がない）
   ...
   - **計画検証**（`--no-plan-review` 未指定時のみ）: Skill ツールで `deep-plan-review` を起動する（引数: 計画ファイルパス）。読者の起動と ExitPlanMode まで同スキルが担うため、本スキル側で重複して呼ばない。読者の検証は承認と並行して走り、その結果の反映は同スキルの承認後の同期点で行われる（実装フェーズの最初のステップより前）
   - **ExitPlanMode**（`--no-plan-review` 指定時のみ）: 参照・コマンドチェックが clean になった時点で本スキルが `ExitPlanMode` を呼ぶ
   - ユーザーの承認を待つ

4. **実装フェーズへ**（承認後）
   - 以下の「実装フェーズ」を、その冒頭の「開始前の同期点」から実行

### 実装フェーズ

**開始前の同期点**（`--no-plan-review` 未指定時）: `deep-plan-review` の承認後の同期点が完了して同スキルが返るまで、以下のステップを 1 つも開始しない（`--worktree` 指定時は Step 1 をスキップするので、実装そのものが最初のステップになる）。同期点で「打ち切り」が選ばれた場合は本 run をそこで終える — この時点で作られているもの（worktree・ブランチ・計画ファイル）はそのまま残し、再開シナリオに拾わせる。`--delegate-impl` の実装エージェント起動は同期点より後なので影響を受けない。
...
実装フェーズの Step 2〜3 を実装エージェントに委譲する。本ブロック完了後は実装フェーズの Step 2〜3 をスキップして Step 4（品質チェック）へ進む。
...
- 作業範囲: 実装・テスト（想定コミット計画の単位で都度コミット）、Test/Lint 成功確認（失敗時は修正 → コミット → 再テスト）
  ...
  - テストコードの作成・修正では Skill ツールで `test-implementation` を起動し、その3原則に従う（実装フェーズの Step 2 の同項は親がスキップするため、委譲時はこの契約が唯一の経路）
...
- **禁止事項**: push・PR 作成・PR の Ready 化（`gh pr ready`）・/simplify は行わない（親が実施。push は後続のレビュー修正フェーズで親が SendMessage で明示指示した場合のみ）
...
- ユーザーに停止を報告する場合、PR が既に作成済みなら 6-3 に到達していないため draft のまま残ることを明記する
...
- サブエージェント失敗時（Agent ツールが null/error を返した場合）の扱いは 6-2 冒頭と同じ（自動リトライしない）

**完了時の親の確認**
- 実装報告を受領後、`git log --oneline` と `git diff --stat` で計画どおりのコミット構成かを確認する（diff 内容の正当性確認は Step 4 の /simplify と Step 6 のレビューが担うため、ここでは全文通読しない）。計画外の修正コミットがある場合は実装報告の記録（テスト失敗起因か）と突合する
...
5. **実装完了処理**
   ...
   - **draft 不変条件の確認**（PR 作成/更新後に無条件で実行。`gh pr view --json number,isDraft` と `gh repo view --json nameWithOwner -q .nameWithOwner` で `<pr-number>` / `<owner/repo>` を確定し、6-1・6-3 でも取り直さず使い回す）:
     - `isDraft` が `false` なら `gh pr ready --undo <pr-number> -R <owner/repo>` で draft に戻し、戻した旨を1行報告する（ユーザー確認は取らない）。`/git-pr` は既存 PR の更新経路で draft 化しないため、再開シナリオ・手動作成の PR はこの確認が回復を担う（ステップ5を経由しない再開は 6-1 で同じ確認を行う）
...
6. **独立セッションでのレビュー → 親での自動修正**
   ...
   - サブエージェントへのプロンプトに以下を含める:
     ...
     - Skill ツールで `deep-review` を引数 `<pr-number> --issue <issue-number> --no-autofix` で起動すること
       - `<pr-number>`: ステップ5で確定した PR 番号（PR 作成後に中断した再開セッション等、同一セッションでステップ5を経由していない場合は、ステップ5と同じ手順で `<pr-number>` / `<owner/repo>` を確定し、draft 不変条件の確認もレビュー起動前に済ませる）
   ...
   - 対応リストの確定後:
     1. **対応すべきものがあれば**: working tree に修正適用 → コミット → テスト・Lint → Skill ツールで `git-pr` を引数 `--base <base-branch>` で起動してプッシュ・PR更新（その「既存PRがある場合（更新判定）」が書き込み直前に説明を取り直す） → 6-3 へ進む
        ...
        - `--delegate-impl` 時: 対応リスト（指摘 + 修正方針）を SendMessage で実装エージェントに送り、修正適用 → コミット → テスト・Lint → プッシュまでを実施させる（実装委譲ブロックの禁止事項のうち push をここで解除する。停止条件は実装委譲ブロックと同じ）。完了後、親が修正コミットの diff を確認して意図どおりか検証し、PR 説明の更新（要否判断を含む）は親が Skill ツールで `git-pr` を引数 `--base <base-branch>` で起動して実施する（直接の編集は「既存PRがある場合（更新判定）」の取り直しを飛ばすため）
   ...
   6-3. **PR を Ready 化**
   - **6-2 の完了後は常に実行する**（指摘を適用・プッシュした場合も、対応すべきものがゼロだった場合も）。`--delegate-impl` 時も親セッションが実行する（実装エージェントには行わせない）
   - 先に同期を検証し、1つでも満たさなければ **Ready 化せず**、満たさなかった項目と PR が draft のまま残ることを報告して停止する（レビュー修正がローカルにだけ・PR が追わない branch にだけ存在する状態で「Ready」を宣言しないための門）:
     ...
     - 検証するのはコミットの同期のみ。テスト・Lint は 6-2 でプッシュ前に実行済みのため再実行しない
...
- **`--delegate-impl` の前提と限界**: deep-plan-review が保証する計画の自己完結性が handoff 品質の前提となる（その保証が揃うのは同スキルの承認後の同期点で、実装エージェントの起動はそれより後になる）。deep-review の指摘数やエスカレーションが目立って増える場合は、委譲をやめる前に計画側の粒度（blocker 基準を「Sonnet が迷わない」水準へ締める）を疑う
- **`--no-plan-review` と `--delegate-impl` の併用は非推奨**: 上記のとおり計画の自己完結性検証が handoff 品質の前提だが、`--no-plan-review` はその同期点ごと省くため、未検証の計画を Sonnet へ委譲することになり品質低下リスクが上がる。両方が指定された場合はその旨を警告した上で続行する（停止はしない）

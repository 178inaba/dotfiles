...
4. **品質チェック**
   - `/simplify` を実行し、変更コードの再利用性・品質・効率性を確認・修正
     ...
     - その検証の後、扱いを確定する前に、Skill ツールで `finding-severity` を起動し、その「判断基準」のうち修正が Issue と矛盾する指摘の行だけを /simplify の finding に適用する（重要度ラベルの行は適用しない。出力は対応リストではなく下記の一覧）。当たる finding を「Issue 側の修正が要る指摘」として集め、空でなければ [references/issue-conflict.md](references/issue-conflict.md) を読んでユーザーに確認し、回答で扱いを決める
     - `/simplify` の要約には、各角度が返したすべての finding を 1 件 1 行で、次の扱いのいずれか 1 つとともに列挙する（列: finding / 角度 / 扱い / 根拠）。一覧はステップ5の開始前に完成させる
       - 適用・見送り: `finding-triage` の「検証根拠の記録」が根拠を求める finding にはその根拠を添える。ユーザーの回答で見送りにした finding は、根拠を検証根拠の代わりに `ユーザー回答: <選択肢>` とする
       - 重複: 重複先の finding をその本文で名指す
       - スコープ外: 変更コード上に無い finding
     - `/simplify` の要約とユーザーの回答はターンの終わりではない。回答が求める Issue 本文の更新・修正とそのコミットもこのステップで済ませる。finding の一覧の完成と修正のコミットまで済ませたら、それ以上のユーザー確認を待たず同一ターンでステップ5へ進む
   - 修正があればコミット

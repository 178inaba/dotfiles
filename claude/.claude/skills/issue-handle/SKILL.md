...
argument-hint: <issue-number | --file FILE_PATH> [--worktree]
...
- `--worktree` 指定時: ベースブランチを最新化してから session を起動すること（`worktree.baseRef: "head"` 設定下では起動時 HEAD から分岐するため、session 起動後の `git fetch` だけでは新規 worktree の起点に反映されない）
...
1. **ベースブランチ取得**: `git branch --show-current` でベースブランチ名を取得して控える

2. **既存 worktree 検出と再開判定**（`--worktree` 指定 & Issue番号指定時のみ）
...
3. **EnterPlanModeツールでPlanモードに移行**（auto mode中でも必ず実行）
...
2. **作業ブランチ確定**
   ...
   **`--worktree` 指定あり**（新規シナリオのみ。再開は事前準備で完了済み）:
   - 計画 branch 名を sanitize（`/` を `-` に置換）して worktree 名とする
     - 例: `feature/99-add-oauth` → `feature-99-add-oauth`
   - **既存 worktree の衝突検出**（safety net）: `git worktree list --porcelain` を解析し、`branch refs/heads/worktree-<worktree-name>` が登録されている worktree を探す
   ...
   - 前提: `worktree.baseRef: "head"` 設定が必要（ベースブランチ = 起動時の現在ブランチの契約を保つため）。session 起動前にベースブランチを最新化しておくこと（session 起動後の `git fetch` は新規 worktree の起点に反映されない）
...
- **`--worktree` 指定時の前提・挙動**:
  - `worktree.baseRef: "head"` 設定が前提（`~/.claude/settings.json`、dotfiles では設定済み）

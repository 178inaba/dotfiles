...
- **worktree 名は branch 名から `/` を `-` に置換した sanitized 形式**（例: `feature/99-add-oauth-login` → `feature-99-add-oauth-login`。規約定義・sanitize 未実施時の実装挙動注記: @~/.claude/skills/worktree-resolution/SKILL.md の「共通規約」）

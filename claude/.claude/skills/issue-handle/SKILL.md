...
6. **独立セッションでのレビュー → 親での自動修正**
   ...
   - 対応リストの確定後:
     - **「Issue 側の修正が要る指摘」が空でなければ**、以下の分岐より前に、1 指摘 1 問の `AskUserQuestion` を 1 ラウンド（ラウンドの定義は `grilling`）で聞き、回答で各指摘を移す。`--file` 指定時は、この区分と選択肢の「Issue」を仕様ファイルと読み替える
       ...
       - 「別 Issue として起票し、ここでは対応しない」→「対応しない指摘」へ。親セッションが `gh issue create -R <owner/repo> --assignee @me --body-file <path>` で単独の Issue（現 Issue の親の Sub にしない）を起票する。本文は指摘と Issue に要る修正で、対象リポジトリの言語で書き、本構成のスキル名を出さない

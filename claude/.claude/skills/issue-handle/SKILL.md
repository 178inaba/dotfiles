...
- `--no-plan-review`: 計画フェーズの計画検証（deep-plan-review）をスキップする。ドキュメント・spec の小修正や単一ファイルの軽微な変更など、計画に blocker も decision も出る余地がほぼなく検証コストが見合わない Issue 向け（`deep-plan-review` は両方で収束するため、blocker だけを見て判断するとスキップが手放すものを過小に見積もる）。完了時の独立セッション `/deep-review` は省略されず、安全網として残る

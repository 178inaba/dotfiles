...
- **作成はスクリプト、切替は EnterWorktree**: worktree の新規作成に `EnterWorktree(name:)` は使わない。切替（`path:`）だけを EnterWorktree に任せる
  ...
  - **スクリプト作成で失われるのは WorktreeCreate hook の発火と終了時の自動クリーンアップ判定のみ**。`.worktreeinclude` コピーは `go/internal/worktree` の共有実装が両経路で再現する（経路ごとに実装を持つとネイティブ挙動の変更時にドリフトするため一本化している）
...
1. **解決**（対象リポジトリ内で実行。`<pr-number>` 省略時はカレント branch の PR を推論）:
   ...
   ```
   出力 JSON のうち本手順で使うフィールド（契約の正は `go/internal/worktree/` のパッケージドキュメント）: `status` / `action` / `worktree_path` / `worktree_name` / `head_ref` / `evacuated` / `synced`（true なら origin へ ff 同期した旨を報告に含める）/ `warnings[]`

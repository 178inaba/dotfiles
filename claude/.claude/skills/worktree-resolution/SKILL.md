...
PR・ブランチに対応する worktree を解決（既存検索・切替・新規作成）するための共通規約と手順。`/issue-handle`・`/review-response`・`/deep-review`・`/understand-pr` の `--worktree` から起動される。PR を対象にレビュー・修正を行うスキルが共有する鮮度確認サブ手順もここに置くので、そのために本スキルは worktree の有無に関わらず起動される。
...
- **作成はスクリプト、切替は EnterWorktree**: worktree の新規作成に `EnterWorktree(name:)` は使わない。切替（`path:`）だけを EnterWorktree に任せる
  ...
  - **スクリプト作成で失われるのは WorktreeCreate hook の発火と終了時の自動クリーンアップ判定のみ**。`.worktreeinclude` コピーは共有実装が両経路で再現する（経路ごとに実装を持つとネイティブ挙動の変更時にドリフトするため一本化している）

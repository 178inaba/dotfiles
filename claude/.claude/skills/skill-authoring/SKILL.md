...
構成例（基本構成テンプレート・良い例・悪い例）は [examples.md](examples.md) を参照。
...
│   └── <topic>.md        # 条件付き詳細・例示の参照ファイル（オプション。内容が分かる名前を付ける）
└── ...
```

参照ファイルは SKILL.md から 1 階層以内に置き、本文からリンクする（入れ子参照は部分読みを誘発する）。100 行を超える場合は先頭に目次を付ける。

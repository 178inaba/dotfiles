...
- **SKILL.md に書くのは「実行コマンド + 自スキルの解釈」だけ**。出力の形・フィールドの意味・exit status は書かない。読者には `ccx <cmd> --help` を案内する
  - フィールド名を書いてよいのは、そこで実際に作用する手順の中だけ。出力の目録は持たない。この目録の規約に機械検査は無い（`ccx skill contract` は実在する名前で組んだ目録を違反にしない。遵守はレビューが担保する）
...
**スキルのステップをそのスキルの外から指すときは、番号ではなく節名で書く**（起動・言及のどちらの行にも掛かる。指す側が他スキルでも rules でも CLAUDE.md でも同じ）。ステップ番号はそのファイル内での位置でしかなく、ステップを1つ足す・消すだけで動くのに、外側の参照がそれに追随する仕組みは無い（実測: 実装フェーズから1ステップ消した変更が5ファイルの番号参照を巻き込んだ）。指したい対象が節そのものではなく節の中の番号付き行なら、節名 + その行が何をするかで指す。**節名に番号の接頭辞（`3B.` 等）が付いていても、引用するときは落とす** — 接頭辞も位置であって名前ではない。スキル内部から自分のステップを番号で指すのはこの規約の対象外（ファイル内番号は Anthropic の Skill authoring best practices が採る形）。
...
本節の規約に機械検査は無い（`@` 添付を取り締まっていた検査は機構ごと廃止した。Anthropic 自身も同種の検査を持たず、遵守はレビューが担保する）。`ccx skill contract` が検査するのは上記「出力契約」の識別子だけで、本節とは別の話。

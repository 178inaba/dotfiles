...
- **SKILL.md に書くのは「実行コマンド + 自スキルの解釈」だけ**。出力の形・フィールドの意味・exit status は書かない。読者には `ccx <cmd> --help` を案内する
  ...
  - `ccx skill contract` が、どのコマンドも公開していない snake_case の名前を違反として報告する
...
| 用途 | 書き方 | 条件・理由 |
|---|---|---|
| **他スキルの本文が要る**（手順として実行する・共有知識をその場で適用する、のいずれも） | 「Skill ツールで `<skill>` を起動する」。先行が必要なら「先に」を付ける。名前は**ディレクトリ名で、先頭のスラッシュは付けない**（プラグイン提供のスキルは `plugin:skill`）。引数は Skill ツールの引数に載せ、呼ばれた側が解釈する散文に残さない。caller が特定の節に依存しているなら「…を起動し、その『PR worktree 解決手順』に従う」のように節名を 1 節添える | 起動時にそのスキル本文と、そのスキルが起動する先が just-in-time で載る。手順スキルと知識スキルで機構を分けない。**起動される側は `disable-model-invocation: true` を持たない** — Skill ツールもサブエージェントの `skills:` プリロードも塞ぐため |
| **言及するだけ**（併用順序・準拠先の案内等） | `/<skill>` または `` `<skill>` `` | その時点で本文が要らない場合に限る。ルールの持ち主を指すだけの引用や、同じ実行で既に起動したスキルへの後方参照がこれ。どちらとも読める場合は起動に倒す（読み込み済みのスキルの再起動は「既に文脈にある」と返るだけで費用がかからない） |

**スキルを部分的に起動することはできない**。手順スキルの一部だけが欲しい場合は、呼ばれる側にフラグを足す（`check-plan-compliance --no-exit`）か、共有部分を知識スキルとして切り出す（`finding-severity`）か、その手順が caller 自身の理由でも必要なら caller が自分の手順として書く。呼ばれる側の判断・手順を caller にコピーするのは選択肢ではない（2 つの写しが 2 人の持ち主を持つとドリフトする）。

検査: `ccx skill contract`（SKILL.md 編集時に実行し、`violations` が空になるまで直す。違反の型は `ccx skill contract --help` にある）。この検査はリポジトリ自身の skills/ に対して CI で走る。

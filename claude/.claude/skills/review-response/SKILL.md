...
- **判断対象の有無の確認**: ドキュメントから `pending`・`reviewers`・`reviews_truncated` を jq で取り出して読み（文書全体を開かずに済ませるのがこのステップの意味）、`pending.threads[]` / `pending.reviews[]` / `pending.comments[]` がすべて空なら「判断対象なし」を1行報告してこの実行（`/loop` では当該イテレーション）を終える。以降のステップは1つも実行せず、判断に達していないので判断の記録も行わない。終了時の報告には `ball: "theirs"` の一覧（内容は「行コメントスレッドの扱い」、`/loop` での省略は「/loop での定期実行」）と、下記のレビュアーの立ち位置を添える。件数の判定は `pending` の3リストだけを出典とし、`reviews[]` / `comments[]` を自前のセレクタで数え直さない — 数え方が2箇所に分かれ、判断対象の有無で食い違うため。`reviewers` を読むのは報告のためだけで、件数の判定には使わない
  - **レビュアーの立ち位置の報告**: `reviewers[]` の各要素を `author` と `state` で1行ずつ書く（実効状態はドキュメントが確定させているので、ここで導出しない）。`author_type` が `Bot` の要素にはその旨を添え、`author` が null（削除済みアカウント）の要素は `(deleted account)` と書く。本文は読まない。`reviews_truncated: true` のときは、`reviewers[]` が取得窓の内側しか表さない旨を併記する。**この報告はゲートで終わる実行にも、ゲートを通った実行の最終報告（「レビュー再依頼」参照）にも載せる** — レビュー待ちの間ユーザーが回すのがこのスキルであり、approve はその待ちが存在する理由そのものなので、実行のたびに現在地が分かる必要がある。`/loop` での省略は「/loop での定期実行」
  - **報告してよい PR の状態はここまで**: ゲートで終わる実行は、上記（3リストの件数・`theirs` の一覧・レビュアーの立ち位置）を超えて PR の状態を述べない。「まだ誰もレビューしていない」「マージできる状態」等の推測を書かない — 直前に取得したドキュメントが `reviewers[]` で答えている問いに、それを読まずに推測で答えると事実と食い違う（2026-09-09 に発生）
...
- **判断と案の報告**: 指摘ごとに以下を報告（出所が本文・行コメント・通常コメントのいずれかを明示）：
  ...
  あわせて「スレッドへの返信と解決」と同じ内容で `threads_path` を書き、`ccx pr reply-threads --dry-run` の出力を返信案と併せて提示する（どのスレッドに何が起きるかを、実行時と同じ検査を通した形で見せるため。この経路は何も投稿しない）。レビュー再依頼の予定も報告する（「レビュー再依頼」参照）。レビュアーの立ち位置（「判断対象の有無の確認」参照）もここに載せる — `--dry-run` はこのステップが最終報告に当たるため
...
**最終報告に載せるもの**: 上記の再依頼の結果に加えて、レビュアーの立ち位置（「判断対象の有無の確認」参照）を必ず載せる。こちらは再依頼と違って `is_own_pr` を条件にしない — 他人の PR でも各レビュアーがどこにいるかは知りたいため、再依頼を行わない実行でも一覧は出す。

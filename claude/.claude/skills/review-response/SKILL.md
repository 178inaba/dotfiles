...
- **コンテキスト取得**: `ccx pr context` で以下を**すべて**一括取得（「指摘・議論経緯の取得」参照）
  ...
  返された `path` が**この実行の唯一のドキュメント**であり、以降のステップはすべてこれを指す。取得は1実行1回で、修正を push した後も取り直さない（投稿するコマンドが自分でローカル HEAD と GitHub の live head を突き合わせるため。「スレッドへの返信と解決」）。判断の記録もこのドキュメントの `fetched_at` を書くので、実行中に届いた指摘が未読のまま retire されることはない
...
**手順**: Skill ツールで `pr-reading` を起動し、その手順に従って読む。起動時に名指すのは、container がドキュメントの `linked_issues[]`、文書がそのドキュメント、差分・コミットの取得元が同じ文書、報告先が下記「報告」。`ahead_own`（未 push のローカル commit がある状態）でも取得元は文書側 — 指摘が付いているのは push 済みの行なので、手元の差分に読み替えない。`comments_truncated: true` の上限の上げ方は下記「指摘・議論経緯の取得」の打ち切り表にある。
...
- **設計思想に関わる指摘**: 意図的な設計判断への異論は、根拠を添えた返信で議論を開くまでを行い、スレッドは解決しない。修正するか押し返すかはユーザーに確認する（合意形成を飛ばして一方的に「修正不要」と確定させない）
  ...
  - **どれにも記録が無ければ、返信を投稿せずエスカレーションが先**。何をどこで探したかを添えてユーザーに提示し、ユーザーが意図を確認したら**先に書き下ろしてから**返信する。書き下ろし先は 2 つ:
    - スコープ・方針に関する決定 → PR 本文。本文を `work_dir` 直下に Write し（このコマンドは置き場所を縛らないが、並列実行が同じ scratchpad を共有するため他の本文ファイルと揃える）、`gh pr edit -R <repo> <PR番号> --body-file <path>` で更新する。`is_own_pr: true` のときだけで、他人の PR では本文を編集せず下のコードコメントかエスカレーションのまま留めるかにする。`--body-file` は追記ではなく**本文全体の置換**なので、渡すファイルにはドキュメントの `pr.body` に節を足した全文を書く（節だけを渡すと既存の本文が消え、誰も気付かない）
...
- `pr` / `repo`: レビュー再依頼のエンドポイント組み立てと、押し返しの根拠を PR 本文に書き下ろすときの `gh pr edit` の `-R` / PR 番号に使用（完了報告の投稿先はコマンドがドキュメントから読むので、こちらで組み立てない）

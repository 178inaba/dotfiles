...
レビュー待ちの間、`/loop 10m /review-response` で本スキルを繰り返し実行できる（新しい指摘が付き次第対応する運用）。各イテレーションは通常モードと同じ手順でよい。未解決の指摘がないイテレーションでは何もせず次の実行を待つ。
...
- **末尾が自分の返信なら「反応待ち」に分類する**: 未解決スレッドのうち `last_comment.author` が `current_user` と一致するものは反応待ちとして分類し、修正要否判断（共通手順4）へは進めない。人間レビュアーのスレッドは対応後も未解決のまま残す方針（下記のポリシー）のため、`is_resolved: false` だけでは対応済みかを判別できないからである。レビュアーがその後に追記したスレッドは `last_comment` が自分の返信でなくなるため、通常どおり対応対象に戻る
  - **適用は `is_own_pr: true` の PR のみ**: 自分がレビュアー側の PR では「末尾が自分」の意味が反転する（相手の応答待ちであり自分の対応漏れではない）ため、この分類を適用しない
...
- `review_threads[]`: 行コメントスレッド。列挙・除外の規定は「行コメントスレッドの扱い」参照。返信・解決 mutation の `THREAD_ID` には `id` を使う。`last_comment` は反応待ち分類の入力（`comments[]` の最終要素とは限らない — 打ち切り時は含まれないため、末尾判定には必ず `last_comment` を使う）
- `current_user`: 反応待ち分類で `last_comment.author` と突き合わせる

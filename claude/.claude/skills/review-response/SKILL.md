...
2. `git fetch origin <head-branch>` の上で、@~/.claude/skills/worktree-resolution/SKILL.md の「共通サブ手順: PR head との鮮度確認」を実行（停止条件に該当した場合は修正・返信を開始しない。stale なコードへの修正適用や、リモートで対応済みの指摘への二重対応を防ぐため。fetch は `--worktree` 時も省略しない — Worktree 解決後の手順1で取得した `pr.head_oid` がその fetch より新しい可能性があるため）
3. 3種類を突き合わせ、重複と通常コメント上で合意済みの議題を除いた上で指摘を列挙
4. 各指摘について修正すべきか判断
5. 修正すべき指摘は実装で対応（コミット・プッシュまで）
6. 修正不要な指摘には理由を説明して返信
7. 対応完了後にコメントを解決済みに変更

### dry-runモード（`--dry-run`）
1. `fetch-pr-context.sh` で**レビュー本文・行コメント・通常コメントのすべて**を一括取得（通常モードと同じ）
2. 「共通サブ手順: PR head との鮮度確認」を実行（通常モードと同じ。dry-run でも同期は行う — 修正案・返信案の起案自体が最新 head を前提とし、同期は作業を破棄しない fast-forward のみのため）
3. 3種類を突き合わせ、重複と通常コメント上で合意済みの議題を除いた上で指摘を列挙
4. 各指摘について修正すべきか判断
...
レビュー本文（`reviews[]`）・レビュースレッド（`review_threads[]`）・通常コメント（`comments[]`）の3種を一括取得し、正規化した JSON を stdout に返す。3つは GitHub 上で別管理のため、一部だけ取得すると本文の指摘（優先度付きリスト等）や通常コメント上の議論経緯・追加依頼を見落とす — 一括取得・`comments` の最新側優先（昇順で返るタイムラインの `last: 50`）・マーカーの先頭一致判定はスクリプトが保証する（挙動の担保: `claude/.claude/tests/test-fetch-pr-context.sh`）。`<pr-number>` 省略時はカレント branch の PR を推論し、失敗時は非ゼロ exit + stderr メッセージで停止する。

...
- `review_threads[]`: 行コメントスレッド。列挙・除外の規定は「行コメントスレッドの扱い」参照。返信・解決 mutation の `THREAD_ID` には `id` を使う。`waiting_for_response: true` は反応待ち（対応対象から外して報告する）、`last_comment` はその報告に載せる末尾コメント（`comments[]` の最終要素とは限らない — 打ち切り時は含まれない）

...
- **`waiting_for_response: true` のスレッドは「反応待ち」に分類する**: 修正要否判断（共通手順4）へは進めない。人間レビュアーのスレッドは対応後も未解決のまま残す方針（下記のポリシー）のため、`is_resolved: false` だけでは対応済みかを判別できないからである。レビュアーがその後に追記したスレッドはフラグが落ちるため、通常どおり対応対象に戻る（判定条件「未解決 かつ 末尾が自分の返信 かつ 自分の PR」は `ccx pr context` が確定させる）

...
5. 修正すべき指摘は実装で対応（コミット・プッシュまで。コミット前に「品質確認」を実行）
6. 修正不要な指摘には理由を説明して返信（「修正不要と判断して返信する場合」・「修正完了報告フォーマット」参照）
7. 対応完了後にコメントを解決済みに変更（「レビュアー別の解決ポリシー」に従う — 人間レビュアーのスレッドは解決しない）

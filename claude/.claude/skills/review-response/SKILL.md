...
3. 3種類を突き合わせ、重複と通常コメント上で合意済みの議題を除いた上で指摘を列挙（列挙・除外・返信先の規定は「ソース別の扱い」に従う）
...
- `review_threads[]`: 行コメントスレッド。列挙・除外の規定は「行コメントスレッドの扱い」参照。返信・解決 mutation の `THREAD_ID` には `id` を使う

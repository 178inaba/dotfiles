...
- **ユーザーが `theirs` のスレッドへの再対応を明示的に指示した場合**、通常のエントリ（`path` + `line`、返信文、`resolve` は当該スレッドの `resolvable_by_me` と同値）で返信する。定常経路と `/loop` が選ぶのは `mine` のみで、「反応待ち」の報告も変わらない。`none` のスレッドは報告に含めるところまでで、**返信・解決はできない**。他人の PR 上で他人のスレッドを閉じる経路は意図的に無く、そちらは /deep-review の「コメントモードON時: 前回指摘スレッドへの返信・解決」（レビュアー側）が閉じる

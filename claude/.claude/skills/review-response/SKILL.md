...
非必須（「推奨」等）と記載されたルールを根拠に見送る場合は、確定前に、変更対象ファイル等の既存コードがそのルールに従っているかも確認する。一様に従っている場合は事実上の必須慣習であり、見送り根拠にならない（確認範囲・判定基準の正: @~/.claude/skills/deep-review/SKILL.md「プロジェクトルール観点」の非必須ルール項目）。

...
スキルが使う配管は `ccx` のサブコマンドで、実装は `go/internal/<domain>/`、cobra 定義は `go/internal/cmd/<group>.go`。**1 つのドメインパッケージを複数のスキルが共有する**ので、パッケージを編集したらそのパッケージのテストを走らせれば足りる。
...
**CI（`.github/workflows/ci.yml`）は `go vet`・golangci-lint・`go test -race` の 3 つだけを実行する**ので、CI の失敗は `go -C go vet ./...` と `go -C go test -race ./...` でそのまま再現できる（lint はローカルに docker を持たないため CI が単一の正）。

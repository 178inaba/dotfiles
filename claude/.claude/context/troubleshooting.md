...
デフォルトは根本原因の解決。症状だけを抑える対処や、問題を迂回するワークアラウンドは原則採用しない。
...
エラーメッセージや推測だけで判断せず、ログ・クエリ・実行順序の確認で現在の状態を把握する。
...
1. **根本原因に対応する** - 症状への対処やワークアラウンドは原則避ける
2. **事実を確認する** - 推測ではなくログ・データで
3. **段階的に絞り込む** - 全体 → 個別 → 最小再現
4. **デバッグコードを削除する** - 解決後に必ず

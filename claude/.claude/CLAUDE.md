...
- **コメント規約**:
  - **内容**: コードを読めば分かる内容は書かない。現在のコードがその形でなければならない理由（不変条件・複雑なアルゴリズムの意図・現存する非自明な回避策）・TODO/FIXME は書く
  - **経緯は書かない**: 以前の状態・置き換えた製品や実装の名前・変更の日付・採らなかった代替案・移行の理由は Issue / PR / コミットメッセージに置く。判定は「このコードベースが最初からこの形だったら、この記述は存在するか」で、No なら書かない。コード内コメントに限らず、リポジトリ内の rules / docs / CLAUDE.md にも同じ判定を適用する
  - 言語は下記「成果物の言語」に従う
...
- **判断密度が高い批評・レビュー・計画・実装**: スキルの明示指定に従う（例: 完了時 deep-review は fable）。スキル外のアドホックな起動では親モデル継承のままで良い
...
- 計画完成後、計画が挙げる参照と検証コマンドの実在確認は `ccx plan check <計画ファイルパス>` を実行する（/deep-plan-review より先。finding が残る計画を検証に回さない）
  - 上記チェックが強制するので、計画を書く時点で守る: これから作る成果物は `(new)` / `（新規）` を注記する（注記の無い新規成果物は実在しない参照として報告される）、挙げた検証コマンドは実行して結果を記録する（実行できないならその理由。形式は `ccx plan check --help`）
...
- **文書化のハードル**: 明文化しないと判断を誤るものだけを書く。文脈から判断できることは書かない（既定は「書かない」。書くと決めたときだけ分離基準で置き場所を選ぶ）。書けるのは現在の制約（何をしない・何が前提か）までで、その制約に至った経緯・採らなかった案は「コメント規約」の経緯の判定に従い Issue / PR に置く。議論の再発は `git log` と closed Issue で引いて防ぐ
- **分離基準**（トリガーの性質で振り分ける）:

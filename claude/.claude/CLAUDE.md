...
- **契約の新規作成/修正にも適用する**: 上記セルフチェックは、選択肢の提示時だけでなく、外部公開契約（DB テーブルスキーマ・DDL、API/IDL 定義。spec・Issue 内の定義を含む）を新規作成・修正するときにも適用する
...
コスト原則: **上位ティアと結果の差が出ないタスクには下位ティアのモデルを明示する**（Sonnet で差が出ない領域は Sonnet、Opus で差が出ない領域は Opus）。この原則を適用してよいのは、下記の調査・探索系サブエージェントと、スキルが明示的に下位ティアのモデルを指定している呼び出しの 2 つだけで、それ以外への一般的な許可としては読まない。
...
- **判断密度が高い批評・レビュー・計画・実装**: スキルの明示指定に従う（例: issue-handle の実装委譲は sonnet、完了時 deep-review は fable）。スキル外のアドホックな起動では親モデル継承のままで良い
  - スキルが `model` を指定していないレビュー・批評（組み込み `/simplify` のレビューエージェントを含む）は、`model` を付けずに起動する（親継承）。呼び出し側の判断で下位ティアへ落とさない — 判断密度の高い角度（altitude 等）のカバレッジが静かに落ち、節約額はサブエージェント込みでも1 Issue あたり数ドルに留まるため

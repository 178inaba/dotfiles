...
- 計画完成後の設計妥当性・自己完結性の検証は /deep-plan-review スキルを使用（/check-plan-compliance と併用時は compliance 先。compliance は `--no-exit` を付けて実行し、ExitPlanMode は deep-plan-review 側に任せる）
